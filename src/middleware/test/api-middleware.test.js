...
import { API_REQUEST, apiRequest, apiSuccess, apiError } from '../../actions/api-actions';
...
describe('api middleware', () => {
    ...
    it('handles api request action and returns a failure action', () => {
        ...
        fetchMock.mock('http://example.com', { status: '404' }, { options: 'GET' })
        return middleware(action).then(() => {
            expect(dispatch).toHaveBeenCalledWith(expected)
        })
    })
    it('skips actions with api request', () => {
        const action = {
            type: 'AN_ACTION'
        }
        middleware(action)
...
const mockMultipeapiResponse = [mockapiResponse[0], mockapiResponse[1], mockapiResponse[0], mockapiResponse[1]];